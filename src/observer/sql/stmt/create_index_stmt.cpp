...
RC CreateIndexStmt::create(Db *db, const CreateIndexSqlNode &create_index, Stmt *&stmt)
{
  ...
  if (is_blank(table_name) || is_blank(create_index.index_name.c_str()) || create_index.attribute_name.size()==0) {
  ...
  for(int i=0;i<create_index.attribute_name.size();++i){
    const FieldMeta *fld = table->table_meta().field(create_index.attribute_name[i].c_str());
    if (nullptr == fld) {
